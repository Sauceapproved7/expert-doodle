...
export function buildCoderNeuralCandidate({
  trainingText,
  evaluationText,
  sourceCommit,
  createdAt,
  ...
  if (trainingTexts.some((text) => !text) || evaluationTexts.some((text) => !text)) {
    throw new Error("Coder neural corpus rows require text");
  }

  const trainingManifest = validateDatasetManifest({
    version: "0.1",
    id: "coder-neural-v02-train",
    description: "Hercules-authored software corpus for Coder neural next-token candidate v0.2.",
    sourceUri: "repo://hercules-training/bootstrap/coder-neural-v0.2-train.jsonl",
    sourceType: "hercules-authored",
    contentSha256: sha256Bytes(Buffer.from(trainingText, "utf8")),
    recordCount: trainingRows.length,
    license: "Apache-2.0",
    rightsBasis: "Original Hercules project software-training material committed in the canonical repository.",
    ...
    provenance: {
      ...
      sourceCommit,
      notes: "Owner-authored JavaScript patterns; no third-party code corpus, weights, tokenizer package, or ML SDK.",
    },
  });
  if (!trainingManifest.ok) {
    throw new Error("Coder training corpus failed provenance gate: " + trainingManifest.errors.join("; "));
  }

  const evaluationManifest = validateDatasetManifest({
    version: "0.1",
    id: "coder-neural-v02-eval",
    description: "Hercules-authored held-out software corpus for Coder neural candidate v0.2.",
    sourceUri: "repo://hercules-training/bootstrap/coder-neural-v0.2-eval.jsonl",
    sourceType: "hercules-authored",
    contentSha256: sha256Bytes(Buffer.from(evaluationText, "utf8")),
    recordCount: evaluationRows.length,
    license: "Apache-2.0",
    rightsBasis: "Original Hercules project held-out software evaluation material committed in the canonical repository.",
    trainingAllowed: true,
    createdAt,
    provenance: {
      origin: "Hercules canonical repository",
      acquiredBy: "Sauceapproved7",
      sourceCommit,
      notes: "Held-out owner-authored code snippets excluded from training.",
    },
  });
  if (!evaluationManifest.ok) {
    throw new Error("Coder evaluation corpus failed provenance gate: " + evaluationManifest.errors.join("; "));
  }
  ...
  const planned = createTrainingJob({
    id: "coder-neural-v02-job",
    modelId: "hercules-coder",
    task: "code",
    ...
    hyperparameters: {
      ...
      corpus: "hercules-coder-software-v0.2",
    },
    createdAt,
  });
  ...
  if (!checkpoint.ok) {
    throw new Error("Coder neural checkpoint invalid: " + checkpoint.errors.join("; "));
  }
  ...
  const suite = validateEvaluationSuite({
    version: "0.1",
    id: "coder-neural-v02-gate",
    task: "code",
    description: "Held-out software-language gate for Hercules Coder neural v0.2.",
    thresholds: [
      {metric: "top1-accuracy", op: "gte", value: 0.42},
      {metric: "content-top1-accuracy", op: "gte", value: 0.08},
      {metric: "unigram-improvement", op: "gte", value: 0.08},
      {metric: "backoff-cross-entropy-ratio", op: "lte", value: 1.45},
      {metric: "token-coverage", op: "gte", value: 0.8},
    ],
    sourceCommit,
  });
  if (!suite.ok) {
    throw new Error("Coder neural evaluation suite invalid: " + suite.errors.join("; "));
  }
  ...
  const evidence = {
    ...
    backoffMetrics,
    samples: [
      generateNeuralText(tokenizer, neuralModel, "export function ", {maxTokens: 32}),
      generateNeuralText(tokenizer, neuralModel, "test(", {maxTokens: 32}),
      generateNeuralText(tokenizer, neuralModel, "if (", {maxTokens: 32}),
    ],
  };

  const evaluation = validateEvaluationResult({
    version: "0.1",
    id: "coder-neural-v02-result",
  ...
  if (!evaluation.ok) {
    throw new Error("Coder neural evaluation result invalid: " + evaluation.errors.join("; "));
  }
...
async function main() {
  ...
  const trainingPath = resolve(
    process.argv[2] ?? "hercules-training/bootstrap/coder-neural-v0.2-train.jsonl",
  );
  const evaluationPath = resolve(
    process.argv[3] ?? "hercules-training/bootstrap/coder-neural-v0.2-eval.jsonl",
  );
  const outDir = resolve(
    process.argv[4] ?? ".hercules-training/coder-neural-v0.2",
  );
  ...
  const build = buildCoderNeuralCandidate({
    trainingText,
    evaluationText,
    sourceCommit,
    createdAt,
  ...
  if (!build.ok) {
    throw new Error("Hercules Coder neural v0.2 missed candidate gate");
  }
