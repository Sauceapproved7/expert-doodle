...
function renderRuntimeStore() {
  return [
    ...
    '    return this.mutate(entity, async ({path, items}) => {',
    '      items[id] = item;',
    '      await atomicWrite(path, items);',
    '      return item;',
    '    });',
    '  }',
    '',
...
function renderServer(spec) {
  ...
  return [
    ...
    'const records = new ForgeRuntimeStore({dataDir, entities});',
    'await records.init();',
    '',
    ...
    '      return json(res, 200, {ok: true, app: "' + spec.name + '", engine: "hercules-forge-owned-core", data: "persistent-file"});',
    '    }',
