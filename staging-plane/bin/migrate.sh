...
for role in staging_anon staging_user; do
  exists="$(psql -At -h postgres -U hercules_admin -d hercules_staging -v role="$role" -c "select count(*) from pg_roles where rolname=:'role'")"
...
for migration in /migrations/*.sql; do
  ...
  applied="$(psql -At -h postgres -U hercules_admin -d hercules_staging -v name="$name" -c "select count(*) from hercules_migrations.applied where name=:'name'")"
  if [ "$applied" = "0" ]; then
    psql -v ON_ERROR_STOP=1 -h postgres -U hercules_admin -d hercules_staging -f "$migration"
    psql -v ON_ERROR_STOP=1 -h postgres -U hercules_admin -d hercules_staging -v name="$name" -c "insert into hercules_migrations.applied(name) values(:'name')"
  fi
done
