...
function fixtureJwtSecret(){
  return Buffer.alloc(48,109).toString("hex");
}

function fixturePassword(){
  return Array.from({length:18},(_,index)=>String.fromCharCode(97+(index%26))).join("")+"1!";
}
...
test("auth migration creates private credential/session tables and server-only RPCs", async () => {
  ...
  assert.match(sql,/create table(?: if not exists)? staging_api\.auth_users/i);
  assert.match(sql,/password_digest text not null/i);
  assert.match(sql,/password_salt text not null/i);
  assert.match(sql,/create table(?: if not exists)? staging_api\.auth_sessions/i);
...
test("refresh keeps the access-token subject bound to the user, not the session", async () => {
  ...
  const previousRefresh=Buffer.alloc(32,114).toString("base64url");
